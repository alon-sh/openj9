 ...
 * */
void loadCryptoLib(J9VMThread *currentThread) {

	if (currentThread->javaVM->jniCryptoFunctions == NULL) {
		...
		if (currentThread->javaVM->jniCryptoFunctions == NULL) {
			...
			CIPHER_t* CIPHER_encryptFinal;
			CIPHER_t* CIPHER_decryptFinal;
			z_kmc_native_t* z_kmc_native;
			...
			j9sl_open_shared_library((char*)cryptolib_name, cryptoLibrary, J9PORT_SLOPEN_DECORATE);
			
			j9sl_lookup_name(*cryptoLibrary, (char*)encryptFinal_name, (UDATA*)&CIPHER_encryptFinal, NULL);
			j9sl_lookup_name(*cryptoLibrary, (char*)decryptFinal_name, (UDATA*)&CIPHER_decryptFinal, NULL);
			j9sl_lookup_name(*cryptoLibrary, (char*)encryptUpdate_name, (UDATA*)&(cryptFuncs->CIPHER_encryptUpdate), NULL);
			j9sl_lookup_name(*cryptoLibrary, (char*)decryptUpdate_name, (UDATA*)&(cryptFuncs->CIPHER_decryptUpdate), NULL);
			j9sl_lookup_name(*cryptoLibrary, (char*)zKMC_name, (UDATA*)&z_kmc_native, NULL);
 ...
 *
 * If the input data is very large and VM Access may be acquired for too extended a duration. In that case, 
 ...
 * Benchmarks indicate current performance at 2020 is at 620 MB/s for AES-CBC encrypt at 16 threads on x86 linux.
...
VMINLINE jint AESLaunch(J9VMThread *currentThread, jlong iccContextId, jlong iccCipherId, j9object_t input, jint inputOffset,jint inputLen, j9object_t output, jint outputOffset, jboolean needsReinit, CIPHER_t* cbcOpUpdate, cryptoFunc_t cbcOp, jlong paramPointer, jint mode) {
...
#ifdef CRYPTO_DEBUG
	printf("** Start Addresses: %p %p %d %d\n",inputAddress, outputAddress, inputIndex, outputIndex);
	printf("** Enter AESLaunch %p %p %d->%d (%lld %lld)\n", inputAddress, outputAddress, inputLen,totalBytes, (long long int)iccContextId, (long long int)iccCipherId);
#endif
	...
	if (totalBytes <= OP_SIZE_LIMIT) {
		...
		int returnCode;
#ifdef CRYPTO_DEBUG
		printf("** %d bytes to process, need to break up %lld\n", totalBytes, (long long unsigned int)paramPointer);
#endif
		jint currentBytes = 0;
		...
		while (0 != bytesToOp) {
			...
			if (paramPointer != 0) {
#ifdef CRYPTO_DEBUG
				printf("** Launch break up zzz_kmc_native with %d bytes, calling %p from %p to %p\n",(int)currentBytes,cbcOp.z_kmc_native,inputAddress,outputAddress);
#endif
...
#ifdef CRYPTO_DEBUG
					printf("** Launch first cbcOp.CIPHER_op with %d bytes, calling %p from %p to %p\n",(int)currentBytes,cbcOpUpdate,inputAddress,outputAddress);
#endif
...
#ifdef CRYPTO_DEBUG
					printf("** Launch subseq but not final cbcOp.CIPHER_op with %d bytes, calling %p from %p to %p\n",(int)currentBytes,cbcOpUpdate,inputAddress,outputAddress);
#endif
...
#ifdef CRYPTO_DEBUG
					printf("** Launch final cbcOp.CIPHER_op with %d bytes, calling %p from %p to %p\n",(int)currentBytes,cbcOp.CIPHER_op,inputAddress,outputAddress);
#endif
...
VMINLINE jint AESLaunch_discontiguous(J9VMThread *currentThread, jlong iccContextId, jlong iccCipherId, j9object_t input, jint inputOffset,jint inputLen, j9object_t output, jint outputOffset, jboolean needsReinit, CIPHER_t* cbcOpUpdate, cryptoFunc_t cbcOp, jlong paramPointer, jint mode) {
...
#ifdef CRYPTO_DEBUG
	printf("*** Allocated %p %p\n",inputNativePtr,outputNativePtr);
#endif
 ...
 */
VMINLINE jint AES(J9VMThread *currentThread, jlong iccContextId, jlong iccCipherId, j9object_t input, jint inputOffset,jint inputLen, j9object_t output, jint outputOffset, jboolean needsReinit, CIPHER_t* cbcOpUpdate, cryptoFunc_t cbcOp, jlong paramPointer, jint mode) {
#ifdef CRYPTO_DEBUG
	printf("** Entered AES routine\n");
#endif
...
#ifdef CRYPTO_DEBUG
	printf("** Ctg. Test %d %d - output array size %d\n", ctgr2_input, ctgr2_output, outputArraySize);
#endif
...
#ifdef CRYPTO_DEBUG
		printf("** Dis-Contiguous Range: exec. AESLaunch\n");
#endif
...
jint JNICALL Fast_com_ibm_crypto_plus_provider_icc_NativeInterface_CIPHER_encryptFinal(J9VMThread *currentThread, jlong iccContextId, jlong iccCipherId, j9object_t plaintext, jint plaintextOffset,jint plaintextLen, j9object_t ciphertext, jint ciphertextOffset, jboolean needsReinit)
{
	bool cryptoLibLoaded = !(currentThread->javaVM->jniCryptoFunctions == NULL);
	if (!cryptoLibLoaded) {
...
jint JNICALL Fast_com_ibm_crypto_plus_provider_icc_NativeInterface_CIPHER_decryptFinal(J9VMThread *currentThread, jlong iccContextId, jlong iccCipherId, j9object_t ciphertext, jint ciphertextOffset,jint ciphertextLen, j9object_t plaintext, jint plaintextOffset, jboolean needsReinit)
{
	bool cryptoLibLoaded = !(currentThread->javaVM->jniCryptoFunctions == NULL);
	if (!cryptoLibLoaded) {
		loadCryptoLib(currentThread);
...
jint JNICALL Fast_com_ibm_crypto_plus_provider_icc_NativeInterface_CIPHER_encryptUpdate(J9VMThread *currentThread, jlong iccContextId, jlong iccCipherId, j9object_t plaintext, jint plaintextOffset,jint plaintextLen, j9object_t ciphertext, jint ciphertextOffset, jboolean needsReinit)
{
	bool cryptoLibLoaded = !(currentThread->javaVM->jniCryptoFunctions == NULL);
	if (!cryptoLibLoaded) {
		loadCryptoLib(currentThread);
...
jint JNICALL Fast_com_ibm_crypto_plus_provider_icc_NativeInterface_CIPHER_decryptUpdate(J9VMThread *currentThread, jlong iccContextId, jlong iccCipherId, j9object_t ciphertext, jint ciphertextOffset,jint ciphertextLen, j9object_t plaintext, jint plaintextOffset, jboolean needsReinit)
{
	bool cryptoLibLoaded = !(currentThread->javaVM->jniCryptoFunctions == NULL);
	if (!cryptoLibLoaded) {
		loadCryptoLib(currentThread);
...
jint JNICALL Fast_com_ibm_crypto_plus_provider_icc_NativeInterface_z_kmc_native(J9VMThread *currentThread, j9object_t input, jint inputOffset, j9object_t output, jint outputOffset, jlong paramPointer, jint inputLength, jint mode) 
{
	bool cryptoLibLoaded = !(currentThread->javaVM->jniCryptoFunctions == NULL);
	if (!cryptoLibLoaded) {
		loadCryptoLib(currentThread);
